...
done < <(mosquitto_sub -h $MQTT_SERVER -p $MQTT_PORT -u "$MQTT_USERNAME" -P "$MQTT_PASSWORD" -i ""$MQTT_DEVICENAME"_"$MQTT_SERIAL"" -t ""$MQTT_TOPIC/sensor/$MQTT_DEVICENAME"_"$MQTT_SERIAL"" -q 1)
